...
- `sandbox.py`: `SecureExecutor` с 5 уровнями защиты и двумя backend (`SubprocessBackend`, заглушка `DockerBackend`).
...
### 3. `DockerBackend` — заглушка

`security/sandbox.py` содержит класс `DockerBackend`, который возвращает:

```python
ToolResult(success=False, stderr="DockerBackend not yet implemented...")
```

### 4. Дублирование проекта
